...
const tabsClass = (active: boolean) =>
    `min-h-11 border-r border-ink/15 px-4 text-left text-[12px] font-black transition-colors last:border-r-0 ${
        active ? 'bg-accent text-ink' : 'bg-white text-ink/55 hover:bg-paper'
    }`
...
export default function ProjectEvidence({ projectId }: { projectId: string }) {
    ...
    if (projectId === 'content-pipeline') return <CmsFlow />
    return null
}
        ...
        <EvidenceFrame label="LMS interaction demo">
            <div className="grid border-b border-ink/20 sm:grid-cols-3">
                {tabs.map((tab, index) => (
                    <button key={tab} type="button" onClick={() => setActive(index)} className={tabsClass(active === index)} aria-pressed={active === index}>
                        {String(index + 1).padStart(2, '0')} · {tab}
                    </button>
                ))}
            </div>
            <div className="grid min-h-[360px] md:grid-cols-[190px_minmax(0,1fr)]">
                <aside className="border-b border-ink/15 bg-[#f7f5ef] p-5 md:border-b-0 md:border-r">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-ink/35">Class 3A · Demo</p>
...
function SyncState() {
    return <>
        <p className="text-[11px] font-bold text-ink/40">LIVE CLASS · 입장 직후 초기 상태</p><h4 className="mt-1 text-2xl font-black">교사 화면과 동기화됨</h4>
...
function AdminDemo() {
    ...
    return <EvidenceFrame label="Admin workflow demo">
        <div className="grid border-b border-ink/20 sm:grid-cols-3">{tabs.map((tab,index)=><button key={tab} type="button" onClick={()=>setActive(index)} className={tabsClass(active===index)} aria-pressed={active===index}>{String(index+1).padStart(2,'0')} · {tab}</button>)}</div>
        <div className="min-h-[360px] bg-[#f7f8fa] p-5 sm:p-7">
...
function AdminHeader({ title, action }: { title: string; action: string }) {
    return <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-ink/35">Account management</p><h4 className="mt-1 text-2xl font-black">{title}</h4></div><button type="button" className="bg-[#3157d5] px-4 py-2 text-[12px] font-black text-white">{action}</button></div>
}
...
function CmsFlow() {
    const automated = ['HTML 수집', '이미지 다운로드', 'WebP 변환', '경로 치환', '언어별 산출물', '다중 등록']
    return <EvidenceFrame label="Content automation flow"><div className="grid md:grid-cols-[0.8fr_1.2fr]"><div className="border-b border-ink/15 bg-[#f7f5ef] p-6 md:border-b-0 md:border-r sm:p-8"><p className="text-[11px] font-black uppercase tracking-[0.14em] text-ink/35">Before</p><h4 className="mt-3 text-2xl font-black">콘텐츠마다 반복 작업</h4><ol className="mt-6 space-y-3 text-[13px] text-ink/60">{['외부 HTML 확보','이미지 개별 저장','배포 경로로 변경','콘텐츠별 등록'].map((item,index)=><li key={item} className="flex gap-3"><b className="text-accent">{index+1}</b><span>{item}</span></li>)}</ol></div><div className="p-6 sm:p-8"><p className="text-[11px] font-black uppercase tracking-[0.14em] text-accent">Automated pipeline</p><h4 className="mt-3 text-2xl font-black">수집부터 등록 준비까지 연결</h4><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3">{automated.map((item,index)=><div key={item} className="border border-ink/20 p-3"><span className="text-[10px] font-black text-accent">{String(index+1).padStart(2,'0')}</span><p className="mt-2 text-[12px] font-black">{item}</p></div>)}</div><div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="bg-ink p-4 text-paper"><p className="text-[10px] font-black text-accent">확인된 구현</p><p className="mt-2 text-[12px] leading-6 text-paper/70">Puppeteer 수집 · 이미지 변환 · 경로 치환 · 다중 등록</p></div><div className="border border-ink/20 p-4"><p className="text-[10px] font-black text-ink/40">측정 원칙</p><p className="mt-2 text-[12px] leading-6 text-ink/65">시간 단축률은 동일 샘플의 재현 측정 전까지 사용하지 않습니다.</p></div></div></div></div><VerifiedScope items={['언어별 산출물 15개', 'WebP 이미지 자산 253개']} note="초기 Canva 등록 샘플의 Git 스냅샷 기준 · 처리 시간 단축률이 아님" /></EvidenceFrame>
}
